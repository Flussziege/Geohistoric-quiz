...
def main():
    ...
    print("Hole Daten aus Wikidata...")
    wikidata_df = query_wikidata(qids)

    if wikidata_df.empty:
        print("Keine Daten aus Wikidata gefunden.")
        return

    # Input-Namen mit Wikidata-Daten verbinden
    df = wikidata_df.merge(input_df, on="qid", how="left")

    # Anzeigename:
    # 1. Name aus CSV, falls vorhanden
    # 2. sonst Wikidata-Label
    # 3. sonst QID
    ...
    output_csv = OUTPUT_DIR / "wikidata_personen_daten.csv"
    df.to_csv(output_csv, index=False, encoding="utf-8")
    print(f"Daten gespeichert: {output_csv}")

    print("Lade Weltkarte...")
    world = gpd.read_file(WORLD_MAP_URL)

    # Länder zu einer Landmasse zusammenfassen:
    # dadurch verschwinden die Ländergrenzen
    land = world.dissolve()

    print("Erzeuge SVG-Karten...")

    for _, row in df.iterrows():
        outpath = draw_person_map(land, row, OUTPUT_DIR, name=qids[_])
